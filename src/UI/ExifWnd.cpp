...
static std::wstring FormatRational(ULARGE_INTEGER ul, const wchar_t* unit = nullptr, int decimals = 2) {
    ...
    wchar_t buf[32];
    if (num == 0) { swprintf_s(buf, L"0%s", unit ? unit : L""); return buf; }
    if (num % den == 0) { swprintf_s(buf, L"%lu%s", num / den, unit ? unit : L""); return buf; }
...
static double GpsDecimal(const PROPVARIANT& pv, const std::wstring& ref) {
    ...
    auto rat = [](ULARGE_INTEGER u) { return u.HighPart ? (double)u.LowPart / u.HighPart : 0.0; };
...
static void CopyToClipboard(HWND hwnd, const std::wstring& text) {
    ...
    if (hMem) {
        ...
        if (ptr) { memcpy(ptr, text.c_str(), bytes); GlobalUnlock(hMem); }
...
std::vector<ExifWnd::ExifRow> ExifWnd::GatherExifRows(
    ...
    if (!app.wicFactory) return rows;

    ComPtr<IWICBitmapDecoder> decoder;
    if (FAILED(app.wicFactory->CreateDecoderFromFilename(
            path.c_str(), nullptr, GENERIC_READ,
            WICDecodeMetadataCacheOnDemand, &decoder))) return rows;

    ComPtr<IWICBitmapFrameDecode> frame;
    if (FAILED(decoder->GetFrame(0, &frame))) return rows;

    ComPtr<IWICMetadataQueryReader> reader;
    if (FAILED(frame->GetMetadataQueryReader(&reader))) return rows;
    ...
    // -- GPS (first — highest priority for phone photos) -----------------
    {
        ...
        if (!lat.empty() || !lon.empty()) {
            ...
            if (pvAlt.vt == VT_UI8 && pvAlt.uhVal.HighPart > 0) {
                ...
                wchar_t b[32]; swprintf_s(b, L"%.1f m %s", alt, below ? L"(below sea)" : L"(above sea)");
                addRow(L"Altitude", b);
            }

            if (pvGpsTime.vt == (VT_VECTOR | VT_UI8) && pvGpsTime.cauh.cElems >= 3) {
                const auto ri = [](ULARGE_INTEGER u) -> UINT   { return u.HighPart ? u.LowPart / u.HighPart : 0; };
                const auto rd = [](ULARGE_INTEGER u) -> double { return u.HighPart ? (double)u.LowPart / u.HighPart : 0.0; };
                ...
                if (gpsDate.size() >= 10 && gpsDate[4] == L':') { gpsDate[4] = L'-'; gpsDate[7] = L'-'; }
                wchar_t tb[16]; swprintf_s(tb, L"%02u:%02u:%04.1f UTC", h, m, s);
                addRow(L"GPS time", gpsDate.empty() ? std::wstring(tb) : gpsDate + L" " + tb);
            }
        }

        PropVariantClear(&pvLatRef);  PropVariantClear(&pvLat);
        PropVariantClear(&pvLonRef);  PropVariantClear(&pvLon);
        PropVariantClear(&pvAltRef);  PropVariantClear(&pvAlt);
        PropVariantClear(&pvGpsTime); PropVariantClear(&pvGpsDate);
    }
    ...
    // -- Camera ----------------------------------------------------------
    {
        ...
        PropVariantClear(&pvMake);  PropVariantClear(&pvModel);
        PropVariantClear(&pvLensMk); PropVariantClear(&pvLensMd);
    ...
    // -- Capture ---------------------------------------------------------
    {
        ...
        if (pvDt.vt == VT_EMPTY) { PropVariantClear(&pvDt); pvDt = q(L"/app1/ifd/{ushort=306}", L"/ifd/{ushort=306}"); }
        ...
        if (hasCapture) {
            ...
            if (!dt.empty()) {
                if (dt.size() >= 10 && dt[4] == L':') { dt[4] = L'-'; dt[7] = L'-'; }
            ...
            if (pvBias.vt == VT_I8) {
                ...
                if (den != 0 && num != 0) {
                    wchar_t b[32]; swprintf_s(b, L"%+.1f EV", (double)num / den);
                    addRow(L"Exp. bias", b);
                }
            }
            ...
            if (pvLS.vt == VT_UI2 && pvLS.uiVal > 0) {
                ...
                switch (pvLS.uiVal) {
                    case 1:   ls = L"Daylight";     break;
                    case 2:   ls = L"Fluorescent";  break;
                    case 3:   ls = L"Tungsten";     break;
                    case 4:   ls = L"Flash";        break;
                    case 9:   ls = L"Fine weather"; break;
                    case 10:  ls = L"Cloudy";       break;
                    case 11:  ls = L"Shade";        break;
                    case 255: ls = L"Other";        break;
                }
            ...
            if (pvCS.vt == VT_UI2) {
                const wchar_t* cs = (pvCS.uiVal == 1) ? L"sRGB"
                                  : (pvCS.uiVal == 0xFFFF) ? L"Uncalibrated" : nullptr;
                if (cs) addRow(L"Color space", cs);
            }
        }

        PropVariantClear(&pvDt);   PropVariantClear(&pvExp);   PropVariantClear(&pvFNum);
        PropVariantClear(&pvIso);  PropVariantClear(&pvFl);    PropVariantClear(&pvFl35);
        PropVariantClear(&pvFlsh); PropVariantClear(&pvProg);  PropVariantClear(&pvBias);
        PropVariantClear(&pvExpM); PropVariantClear(&pvMeter); PropVariantClear(&pvWB);
        PropVariantClear(&pvLS);   PropVariantClear(&pvScene); PropVariantClear(&pvCS);
    }
    ...
    // -- Author / rights -------------------------------------------------
    {
        ...
        PropVariantClear(&pvDesc); PropVariantClear(&pvArtist);
        PropVariantClear(&pvCopy); PropVariantClear(&pvSoft);
...
void ExifWnd::LoadExifData() {
    ...
    m_rows = GatherExifRows(app.playlist[app.currentIndex], app.imgWidth, app.imgHeight);
...
LRESULT ExifWnd::HandleMessage(UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        for (const auto& row : m_rows) {
            if (row.isSection) {
                ...
                RECT vr = { pad + labelW + valGap, y, cR, y + rowH };
    ...
    case WM_SETCURSOR: {
        ...
        if (pt.x < sbX) {
            ...
            for (int i = 0; i < static_cast<int>(m_rows.size()); ++i) {
                ...
                if (pt.y >= y && pt.y < y + h) { hand = !m_rows[i].action.empty(); break; }
                y += h;
            }
        }
    ...
    case WM_RBUTTONDOWN: {
        ...
        if (pt.x >= sbX) {
            ...
            for (int i = 0; i < static_cast<int>(m_rows.size()); ++i) {
                const int h = m_rows[i].isSection ? (sectH + gap) : rowH;
                if (pt.y >= y && pt.y < y + h) { hit = i; break; }
                y += h;
            }
            if (hit >= 0) {
    ...
    case WM_EXIF_READY: {
        auto* rows = reinterpret_cast<std::vector<ExifRow>*>(lParam);
