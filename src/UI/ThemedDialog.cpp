    ...
    switch (msg) {
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDOK:     s_result = 1; s_running = false; return 0;
                case IDCANCEL: s_result = 0; s_running = false; return 0;
            }
            break;
        case WM_CLOSE:
            s_result = 0; s_running = false;
            return 0;
        case WM_ERASEBKGND: {
...
void ThemedDialog::ApplyTheme() {
    ...
    const COLORREF bgColor = Constants::Theme::ThemedGray(
...
int ThemedDialog::RunModal(HWND hOwner, const wchar_t *text, const wchar_t *caption, bool confirm) {
    ...
    auto sc   = [dpi](int logical) { return MulDiv(logical, dpi, 96); };
