...
// PATH HYGIENE lives in HistoryPath.cpp.
//
// SPLIT OUT SO IT CAN BE TESTED. It is the only part of this file that is pure -
// no AppState, no registry, no disk - and it is the part that validates
// UNTRUSTED input: every line of a hand-editable file passes through Normalize,
// which is what rejects control characters, the Win32-illegal set and absurd
// lengths. That is exactly the code a test should cover, and it could not be
// reached while it sat in a translation unit that pulls in the whole app.
//
// Same move, and the same reason, as AddressMatch.cpp out of RemoteSettings.cpp.
