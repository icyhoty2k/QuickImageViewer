...
namespace {
    ...
    // --- Address literals live in AddressMatch.cpp --------------------------
    //
    // StripScope, ParseV4, ParseV6, Cidr, ParseCidr and ParseRange moved there
    // along with the five functions built on them (AddressMatches, InList,
    // SameHost, BlockScope, LooksLikeAddress).
    //
    // WHY: InList decides who may connect, and it could not be unit-tested from
    // here — this file also pulls in AppState, RemoteBlacklist, RemoteCrypto and
    // DedicatedSettings, and a test cannot link half the application. The
    // declarations stayed in RemoteSettings.h, so nothing that calls them
    // changed.
...
// LooksLikeAddress, SameHost, BlockScope, AddressMatches and InList moved to
// AddressMatch.cpp so they could be unit-tested — see the note above where
// their helpers used to be. Declarations are unchanged in RemoteSettings.h.
